...
class PageTwo(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        def onListsButtonsClick(index,title):
            if index == 11:
                date = self.SearchEnrty.get()
                for line in example.get_works_by_date(date):
        ...
        self.SearchEnrty = tk.Entry(self)
        self.SearchEnrty.place(x=1200,y=50)
        ...
        tk.Button(self, text="Поиск по дате",
                  command=lambda: onListsButtonsClick(11,"Поиск по дате")).place(x=1000, y=50)
